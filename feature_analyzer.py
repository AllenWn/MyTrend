...
import json
import os
from features import screen_second_wave, get_wave_periods, calculate_technical_indicators
from config import FEATURE_COMBINATION_PATH, MIN_SUPPORT, MAX_COMBINATION_LEN, NEG_THRESHOLD  # 从config导入阈值配置
...
class FeatureCombinationAnalyzer:
    def __init__(self, min_support=None, max_combination_len=None, neg_threshold=None):
        ...
        self.max_combination_len = max_combination_len if max_combination_len is not None else MAX_COMBINATION_LEN
        self.neg_threshold = neg_threshold if neg_threshold is not None else NEG_THRESHOLD
    ...
    def analyze_positive_samples(self, positive_data_list):
        ...
        # 按支持度排序
        self.positive_patterns = sorted(qualified_patterns, key=lambda x: x["support"], reverse=True)
        
        # 显示找到的特征组合及其语义内容
        print(f"\n找到 {len(self.positive_patterns)} 个符合条件的特征组合:")
        for i, pattern in enumerate(self.positive_patterns[:10]):  # 显示前10个
            semantic_content = self._display_feature_combination_semantics(pattern["features"])
            print(f"  组合{i+1}: {semantic_content}")
            print(f"      特征: {pattern['features']}")
            print(f"      支持度: {pattern['support']:.3f} ({pattern['count']}/{total_samples})")
    ...
    def validate_with_negative_samples(self, negative_data_list):
        ...
        for pattern in self.positive_patterns:
            ...
            neg_frequency = neg_count / total_neg_samples if total_neg_samples > 0 else 0
