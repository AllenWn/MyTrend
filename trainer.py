...
from sklearn.metrics import precision_score, recall_score, f1_score, classification_report, confusion_matrix, roc_auc_score # 新增
from config import MIN_IMPROVEMENT
def train_model(model, train_loader, test_loader, criterion, optimizer, device, epochs, patience, MIN_IMPROVEMENT):  # 新增MIN_IMPROVEMENT参数
    ...
    return model, {'f1': best_f1, 'recall': recall, 'false_positive_rate': false_positive_rate}
...
def evaluate_model(model, test_loader, device):
    ...
    try:
        if len(np.unique(all_labels)) == 2:
            auc = roc_auc_score(all_labels, [p[1] for p in F.softmax(torch.tensor(all_preds), dim=1).numpy()])
            print(f"\nAUC: {auc:.4f}")
    except:
        print("\n无法计算AUC")
...
def check_data_distribution(data_loader):
    ...
    print(f"数据分布: 正样本 = {positive} ({positive/total*100:.2f}%), 负样本 = {negative} ({negative/total*100:.2f}%)")
