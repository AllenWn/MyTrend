    ...
    def _load_from_cache(self):
        ...
        if os.path.exists(self.cache_path):
            try:
                ...
                self.last_updated = (
                    df["last_updated"].iloc[0] if "last_updated" in df.columns else None
                )
                print(
                    f"成功从缓存加载交易日历，共{len(self.trading_days)}个交易日，最后更新时间: {self.last_updated}"
                )
                return True
            except Exception as e:
    ...
    def get_trading_days(self, start_date, end_date):
        ...
        try:
            return [
                ...
                if start_date_standard <= d <= end_date_standard
            ]
        except Exception as e:
            print(f"日期比较失败: {e}")
            return []
