...
class KeyFeatureModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes, key_feature_idx, 
                 weight_mode="dynamic"):
        super().__init__()
        ...
        if weight_mode == "dynamic":
            self.feature_weights = nn.Parameter(torch.ones(input_size))  # 可学习的特征权重
        else:
        ...
        self.attention = nn.Sequential(
            nn.Linear(hidden_size * 2, 1),  # 双向LSTM输出维度为2*hidden_size
            nn.Tanh()
        )
    ...
    def forward(self, x):
        ...
        if self.weight_mode == "dynamic":
            # 动态模式：对每个特征应用可学习的权重
            weighted_x = x * self.feature_weights.unsqueeze(0).unsqueeze(0)  # 广播到(batch, seq_len, input_size)
        ...
        weighted_sum = torch.bmm(attn_weights.transpose(1, 2), lstm_out).squeeze(1)  # (batch, 2*hidden_size)
