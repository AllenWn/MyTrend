...
def get_wave_periods(df):
    ...
    # 金叉/死叉（1=金叉，-1=死叉）
    valid_df['ma_cross'] = 0
    valid_df.loc[(valid_df['ma5'] > valid_df['ma10']) 
                 & (valid_df['ma5'].shift(1) <= valid_df['ma10'].shift(1)), 'ma_cross'] = 1
    valid_df.loc[(valid_df['ma5'] < valid_df['ma10']) 
                 & (valid_df['ma5'].shift(1) >= valid_df['ma10'].shift(1)), 'ma_cross'] = -1
    ...
    valid_df['ma5_dir'] = valid_df['ma5_angle'].apply(lambda x: 1 if x > 0.5 else (-1 if x < -0.5 else 0))
    valid_df['ma10_dir'] = valid_df['ma10_angle'].apply(lambda x: 1 if x > 0.5 else (-1 if x < -0.5 else 0))
    ...
    # 3.3 震荡波段（交叉频繁，方向混乱）
        ...
        dir_change = (valid_df['ma5_dir'].diff() != 0).sum() + (valid_df['ma10_dir'].diff() != 0).sum()
    ...
    if not bands:
        # 取最后一次金叉/死叉作为波段起点
        last_cross_idx = valid_df[valid_df['ma_cross'] != 0].index[-1] if (valid_df['ma_cross'] != 0).any() else None
...
def check_drawdown_controlled(input_data):
    ...
    if overall_trend in ['持续多头', '上涨波段']:
        ...
        current_low = adjustment_data['low'].min() if not adjustment_data.empty else band_data['low'].min()
...
def check_break_adjustment_high(input_data):
    ...
    if overall_trend in ['横盘整理', '震荡波段']:
        ...
        return (current_day['close'] > recent_high and prev_close > recent_high) and volume_condition
...
def check_break_first_wave_high(input_data):
    ...
    volume_condition = periods['current_day']['volume'] > 1.3 * first_wave_data['volume'].mean()
...
def check_volume_pattern(input_data):
    ...
    if overall_trend in ['持续多头', '上涨波段']:
        ...
        adjustment_mask = df['close'].iloc[band_start:band_end+1] < peak_price * 0.95
        if adjustment_mask.any():
            ...
            first_vol_mean = first_wave_data['volume'].mean() if not first_wave_data.empty else adj_vol_mean
    ...
    # 3. 其他趋势（震荡/下跌）保留量价齐升核心判断
        ...
        # 至少满足量价齐升基本条件
        return current_day['close'] > prev_day['close'] and current_day['volume'] > prev_day['volume']
...
def check_ma_bullish(input_data):
    ...
    if band_type in ['持续多头', '上涨波段']:
        # 上涨趋势中要求均线角度为正（趋势延续）
        return is_basic_bull and \
               periods['ma_metrics']['ma5_angle'] > 0 and \
               periods['ma_metrics']['ma10_angle'] > 0
    elif band_type == '横盘整理':
        # 横盘突破时要求短期均线角度转正
...
def check_ma_golden_cross(input_data):
    ...
    if last_cross == 1:
        ...
        if periods['overall_trend'] in ['持续多头', '上涨波段']:
            # 上涨趋势中，金叉需伴随均线角度增大
            return periods['ma_metrics']['ma5_angle'] > periods['ma_metrics']['ma10_angle']
        elif periods['overall_trend'] == '横盘整理':
            # 横盘趋势中，金叉需突破整理区间
            current_band = periods['bands'][-1]
            recent_high = df['high'].iloc[current_band['start']:current_band['end']].max()
            return periods['current_day']['close'] > recent_high
        else:  # 震荡波段
            # 震荡中，金叉需伴随成交量放大
            return periods['current_day']['volume'] > 1.2 * df['volume'].iloc[-5:].mean()
...
def check_macd_second_red(input_data):
    ...
    if overall_trend in ['持续多头', '上涨波段']:
        ...
        is_near_zero = (adjustment_data['macd_hist'].abs() < 0.5 * first_macd_peak).all()
...
def check_rsi_breakout(input_data):
    ...
    if overall_trend in ['持续多头', '上涨波段']:
        ...
        if adjustment_mask.any():
            ...
            return (adj_rsi >= 30).all() and 60 < df['rsi14'].iloc[current_idx] < 80  # 新增超买过滤
    ...
    # 3. 震荡波段（近5天RSI未超卖，当前突破60）
        ...
        recent_rsi = df['rsi14'].iloc[-5:] if current_idx >=4 else df['rsi14'].iloc[:current_idx+1]
...
def check_bb_upper_break(input_data):
    ...
    if overall_trend in ['持续多头', '上涨波段']:
        ...
        if current_idx < 1:
            return is_break
        return is_break and df['close'].iloc[current_idx - 1] > df['bb_upper'].iloc[current_idx - 1]
    else:  # 震荡波段
        # 震荡中突破需成交量显著放大（1.5倍）
...
def check_bull_engulfing(input_data):
    ...
    if not periods['is_valid'] or periods['current_day'] is None or periods['prev_day'] is None:
        return False
    ...
    is_engulf = current_day['open'] < prev_day['close'] and current_day['close'] > prev_day['open']
    ...
    if base_pattern:
        if overall_trend == '震荡波段':
            ...
            # 横盘突破需收盘价创新高
            return current_day['close'] > df['high'].iloc[periods['bands'][-1]['start']:periods['current_idx']].max()
...
def is_price_volume_rise(input_data):
    ...
    if base_condition:
        # 计算近期平均成交量作为参考基准
        recent_vol_mean = df['volume'].iloc[-10:].mean() if len(df) >= 10 else prev['volume']
...
def is_big_bull_candle(input_data):
    ...
    if base_condition:
        # 起爆点需处于关键位置（波段起点或突破点）
        is_crucial_point = (
            current_band['start'] == periods['current_idx']  # 波段第一天
            or periods['current_period']['type'] in ['第二波上涨', '突破调整区间']  # 突破阶段
        )
        
        if overall_trend in ['持续多头', '上涨波段']:
            # 上涨趋势中，大阳线需伴随量能放大且处于调整后
            return is_crucial_point and current['volume'] > 1.3 * df['volume'].iloc[-10:].mean()
        elif overall_trend in ['横盘整理', '震荡波段']:
            # 横盘/震荡中，大阳线需突破波段高点
            band_high = df['high'].iloc[current_band['start']:current_band['end']].max()
...
def is_break_flag_pattern(input_data):
    ...
    is_flag = (
        ...
        and (adjustment_highs.max() - adjustment_lows.min()) / adjustment_lows.min() < 0.05  # 振幅小
    )
...
def check_ma_convergence(input_data):
    ...
    if (abs_diff_5_10 < ABSOLUTE_THRESHOLD and 
        abs_diff_5_20 < ABSOLUTE_THRESHOLD and 
        abs_diff_10_20 < ABSOLUTE_THRESHOLD):
        return True
    ...
    if overall_trend in ['持续多头', '上涨波段']:
        ...
        prev_prev_pct = abs(prev_prev_ma5 - prev_prev_ma10) / prev_prev_min if prev_prev_min > 0 else 1
...
if __name__ == "__main__":
    ...
    adjustment = first_wave[-1] * (1 - np.random.uniform(0.3, 0.5, 19)) + np.random.randn(19)*0.5
    ...
    df = pd.DataFrame({
        ...
        'volume': np.concatenate([np.random.randint(1000,2000,20), np.random.randint(500,800,19), [1500]])
    }, index=date_range)
