...
from config import (
    FEATURE_COLS, INPUT_WINDOW, OUTPUT_WINDOW, 
    INITIAL_START_DATE, INITIAL_END_DATE, FUTURE_RETURN_THRESHOLD
)
...
def extract_raw_labeled_data():
    ...
    print(f"\n标签分布分析：")
    print(f"  训练集 - 正样本(1): {sum(train_labels)}, 负样本(0): {len(train_labels) - sum(train_labels)}")
    print(f"  测试集 - 正样本(1): {sum(test_labels)}, 负样本(0): {len(test_labels) - sum(test_labels)}")
...
def save_raw_data(train_sequences, test_sequences):
    ...
    stats = {
        ...
        'train_positive_ratio': sum([label for _, label in train_sequences]) / len(train_sequences),
        'test_positive_ratio': sum([label for _, label in test_sequences]) / len(test_sequences),
        'input_window': INPUT_WINDOW,
        'output_window': OUTPUT_WINDOW,
