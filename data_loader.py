...
from config import (FUTURE_RETURN_THRESHOLD, FEATURE_COLS, INPUT_WINDOW, INITIAL_START_DATE, INITIAL_END_DATE, 
                   DERIVED_FEATURE_DAYS, SCALER_DIR)

class KeyFeatureDataLoader:
    def __init__(self, data_path, feature_cols, key_feature_name, label_col, 
                 sector_code=None, start_date=None, end_date=None,
                 stocks_file=None, trading_calendar=None,
                 data_dir='stock_data', cache_dir=None, use_baostock=False,
    ...
    def get_sector_stocks(self, sector_code=None):
        ...
        try:
            ...
            stocks = [{'code': row['code'], 'name': row.get('name', '')} for _, row in df.iterrows()]
...
# data_loader.py v2.0（完整版本 - 3/5）
    def get_stock_data(self, symbol, from_date, to_date, max_retries=5, delay=2):
        ...
        trading_days = self.calendar.get_trading_days(
            self._format_date(formatted_from_date), 
            self._format_date(formatted_to_date)
        )
        if not trading_days:
            print(f"警告：无交易日数据")
            return pd.DataFrame()
    ...
    def _add_technical_indicators(self, data):
        ...
        # 处理技术指标中的异常值
        technical_features = ['ma5', 'ma10', 'vol5', 'vol10', 'rsi', 'bb_upper', 'bb_lower']
        for feature in technical_features:
            if feature in data.columns:
                ...
                data[feature] = data[feature].clip(lower=mean - 5*std, upper=mean + 5*std)
...
# data_loader.py v2.0（完整版本 - 4/5）
    def preprocess_data(self, data, target_type='close', print_stats=True):
        ...
        if print_stats:
            ...
            for col in FEATURE_COLS:
                print(f"{col}：均值={valid_data[col].mean():.4f}，标准差={valid_data[col].std():.4f}")
                
                
            print("\n===== 标准化后特征统计 =====")
            for i, col in enumerate(FEATURE_COLS):
                med = np.median(normalized_data[:, i])
                iqr = np.percentile(normalized_data[:, i], 75) - np.percentile(normalized_data[:, i], 25)
                print(f"{col}：中位数={med:.4f}，四分位距={iqr:.4f}")
                
        
        normalized_df = pd.DataFrame(normalized_data, columns=FEATURE_COLS, index=data.index)
    ...
    def create_sequences(self, data, input_window, output_window, target_type='close'):
        ...
        for i in range(L - input_window - output_window):
            ...
            future_prices = data_np[i+input_window : i+input_window+output_window, target_col_idx]  # 未来5天价格
    ...
    def create_feature_screening_data(self, data, input_window, output_window, target_type='close'):
        """
        ...
        if L < total_days_needed + output_window:
            print(f"警告：数据长度{L}不足{total_days_needed + output_window}天，无法生成特征筛选数据")
            return []
        ...
        for i in range(L - total_days_needed - output_window):
            ...
            future_prices = data.iloc[i+total_days_needed : i+total_days_needed+output_window][target_type]  # 未来5天价格
            ...
            # 检查数据是否有异常值
            if feature_data.isnull().any().any() or feature_data.isin([np.inf, -np.inf]).any().any():
                print(f"警告：序列{i}包含异常值，已跳过")
                continue
        ...
        print(f"每个样本包含 {total_days_needed} 天数据（{DERIVED_FEATURE_DAYS}天技术指标 + {input_window}天输入窗口）")
        positive_count = sum(1 for item in feature_data_list if item['label'] == 1.0)
        print(f"正样本数量: {positive_count}, 负样本数量: {len(feature_data_list) - positive_count}")
        
        return feature_data_list

    def create_sequences_with_raw_data(self, data, input_window, output_window, target_type='close'):
        """
        ...
        if target_type not in self.processed_columns:
            raise ValueError(f"目标类型{target_type}不在特征列中: {self.processed_columns}")
        target_col_idx = self.processed_columns.index(target_type)
        ...
        if not sequences:
            raise ValueError(f"未生成有效训练序列，数据长度需≥{total_input_len + output_window}天（40+5=45天）")
    ...
    def prepare_raw_data_for_training(self, input_window, output_window, 
                                     train_test_split=0.8, target_type='close'):
        """
        ...
        # 获取板块成分股数据
        df, valid_symbols = self.get_sector_stock_data(
        ...
        for symbol in tqdm(valid_symbols, desc="处理成分股原始数据"):
            try:
                ...
                # 确保所有必要特征都存在
                missing_features = [col for col in FEATURE_COLS if col not in stock_data.columns]
                if missing_features:
                ...
                for col in FEATURE_COLS:
                    if stock_data[col].isna().all():
                        ...
                        stock_data[col] = stock_data[col].ffill().fillna(stock_data[col].mean())
                ...
                # 生成训练样本（使用原始数据）
                sequences = self.create_sequences_with_raw_data(stock_data[FEATURE_COLS], input_window, output_window, target_type)
                if not sequences:
        ...
        print(f"已准备{len(train_sequences)}个训练样本和{len(test_sequences)}个测试样本（原始数据）")
...
# data_loader.py v2.0（完整版本 - 5/5）
    ...
    def validate_stock_data(self, data, required_cols):
        ...
        print(f"数据验证通过，共{len(data)}行，时间范围: {data.index.min().strftime('%Y-%m-%d')} 至 {data.index.max().strftime('%Y-%m-%d')}")
        return True
    ...
    def create_dataset(self, stock_data_dict, input_window, output_window, target_type='close'):
        """为板块内所有股票创建训练数据集"""
        ...
        for symbol, stock_info in tqdm(stock_data_dict.items(), desc="创建数据集"):
            ...
            try:
                # 预处理数据（标准化）
                processed_data, scaler = self.preprocess_data(data, target_type=target_type)
    ...
    def create_feature_screening_dataset(self, stock_data_dict, input_window, output_window, target_type='close'):
        """
        为板块内所有股票创建特征筛选数据集
        ...
        for symbol, stock_info in tqdm(stock_data_dict.items(), desc="创建特征筛选数据集"):
            name = stock_info['name']
            data = stock_info['data']
            ...
            try:
                ...
                if not self.validate_stock_data(data, ['open', 'close', 'high', 'low', 'volume']):
                ...
                if feature_data_list:
                    feature_screening_data[symbol] = {
                        ...
                        'positive_samples': sum(1 for item in feature_data_list if item['label'] == 1.0),
                        'negative_samples': sum(1 for item in feature_data_list if item['label'] == 0.0)
                    }
        ...
        # 统计总体情况
        total_samples = sum(info['total_samples'] for info in feature_screening_data.values())
        total_positive = sum(info['positive_samples'] for info in feature_screening_data.values())
        total_negative = sum(info['negative_samples'] for info in feature_screening_data.values())
    ...
    def get_sector_stock_data(self, from_date, to_date, max_retries=3, force_download=False):
        ...
        # 计算日期范围内的交易日数量
        trading_days = self.calendar.get_trading_days(
            self._format_date(from_date), 
            self._format_date(to_date)
        )
        if not trading_days:
        ...
        for i, stock in enumerate(tqdm(stocks, desc="获取成分股数据")):
            symbol = stock['code']
            try:
                stock_data = self.load_or_append_data(symbol, from_date, to_date, force_download)
                ...
                if (i + 1) % 20 == 0:  # 每请求20只股票后暂停
                    print(f"已请求{i+1}/{len(stocks)}只股票，暂停3秒避免请求频率超限...")
                    time.sleep(3)  # 暂停3秒
    ...
    def prepare_sector_data(self, start_date, end_date, force_download=False):
        ...
        try:
            # 加载板块内所有股票数据
            print(f"开始加载{self.sector_code}板块数据，时间范围: {start_date} 至 {end_date}")
    ...
    def prepare_sector_data_for_training(self, input_window, output_window, 
                                         train_test_split=0.8, target_type='close'):
        """
        ...
        for symbol in tqdm(valid_symbols, desc="处理成分股数据"):
            try:
                ...
                # 生成训练样本
                sequences = self.create_sequences(normalized_data, input_window, output_window, target_type)
                if not sequences:
                    print(f"警告：股票{symbol}生成的序列为空，跳过")
                    continue
        ...
        print(f"已准备{len(train_sequences)}个训练样本和{len(test_sequences)}个测试样本")
        return train_sequences, test_sequences
