...
class StockPredictor:
    ...
    def _init_tushare(self):
        ...
        try:
            ...
            try:
                api.daily(
                    ts_code="600000.SH", start_date="20230101", end_date="20230102"
                )
    ...
    def _load_and_format_data_OLD(self, symbol, start_date, end_date):
        ...
        # 若日期在索引中，转换为列
        if "date" not in stock_data.columns and isinstance(
    ...
    def preprocess_stock_data_OLD(
        self, stock_data, symbol, target_date, input_window=INPUT_WINDOW
    ):
        ...
        stock_trading_days = stock_data[stock_data["date"].isin(valid_trading_days)][
            "date"
        ]
        if len(stock_trading_days) < input_window:
            raise ValueError(
                f"有效交易日不足{input_window}天（实际{len(stock_trading_days)}天）"
            )
        ...
        # 6. 校验所有10个特征是否存在
        missing_features = [
            col for col in self.feature_cols if col not in stock_data.columns
        ]
        if missing_features:
        ...
        if scaler.n_features_in_ != len(self.feature_cols):
            raise ValueError(
                f"{symbol}的scaler特征数量不匹配（预期{len(self.feature_cols)}，实际{scaler.n_features_in_}）"
            )
    ...
    def preprocess_stock_data(
        ...
        if os.path.exists(scaler_path):
            try:
                ...
                add_safe_globals(
                    [MinMaxScaler, np._core.multiarray._reconstruct, np.dtype]
                )
                ...
                if scaler.n_features_in_ != len(self.feature_cols):
                    raise ValueError(
                        f"scaler特征数量不匹配（预期{len(self.feature_cols)}，实际{scaler.n_features_in_}）"
                    )
            except Exception as e:
    ...
    def predict_stock(self, stock_data, symbol, target_date, input_window=INPUT_WINDOW):
        ...
        # 检查数据完整性
        is_complete, data_msg = self._check_data_integrity(
            stock_data, target_date, input_window
        )
        if not is_complete:
    ...
    def find_qualified_stocks(
        self, target_date=None, threshold=0.6, input_window=INPUT_WINDOW
    ):
        """寻找符合条件的股票"""
        # 确定目标日期
        ...
        for stock in tqdm(stocks, desc="预测成分股"):
            ...
            try:
                ...
                is_qualified, prob, status = self.predict_stock(
                    ...
                    symbol,
                    target_date,
                    input_window,  # 用配置中的INPUT_WINDOW
                )
                ...
                if status == "正常" and is_qualified and prob >= threshold:
                    ...
                    feature_matches = self._calculate_feature_matches(
                        stock_data, symbol
                    )
        ...
        print(
            f"预测完成，找到{len(qualified)}只符合条件的股票,找到{len(qualified_feature)}只符合条件的股票"
        )
    ...
    def _calculate_feature_matches(self, stock_data, symbol):
        ...
        try:
            ...
            if len(stock_data) < 40:
                ...
                extended_data = self._load_and_format_data(
                    symbol,
                    extended_start,
    ...
    def save_prediction_results(self, results, target_date):
        ...
        output_file = os.path.join(
            output_dir, f"prediction_{target_date.replace('-', '')}.csv"
        )
        ...
        for group_name, group_stocks in probability_groups.items():
            if group_stocks:
                ...
                for stock in group_stocks[:10]:  # 只显示前10只
                    print(
                        f"  {stock['code']} {stock['name']} - 概率:{stock['probability']:.4f} 特征匹配:{stock['feature_matches']}"
                    )
...
def main():
    try:
        ...
        qualified_stocks, qualified_feature = predictor.find_qualified_stocks(
            target_date, threshold=0.6
        )
        ...
        if qualified_stocks:
            ...
            for stock in qualified_stocks:
                print(
                    f"{stock['code']} {stock['name']} - 上涨概率: {stock['probability']:.4f}"
                )
        ...
        if qualified_feature:
            ...
            for i, stock in enumerate(qualified_feature[:20], 1):  # 显示前20只
                print(
                    f"{i:2d}. {stock['code']} {stock['name']} - 上涨概率: {stock['probability']:.4f} 特征匹配: {stock['feature_matches']}"
                )
